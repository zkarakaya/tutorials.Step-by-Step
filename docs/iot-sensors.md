...
* A **Smart Lamp** can be switched on or off remotely. It can also report on its current state (`ON` or `OFF`).
  ...
  Furthermore the device can report on the current luminosity of the bulb.
...
HTTP requests generated by the Context Broker and passed downwards towards an IoT device (via
an IoT agent) are known as southbound traffic. Southbound traffic consists of **commands** sent to
...
HTTP POST can also be used to report new measures to an IoT Agent. Again the path will be `/iot/d`, but in this case, `d` (Data) is not necessary -
